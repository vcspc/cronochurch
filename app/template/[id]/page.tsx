...
import { ArrowLeft, Edit3, Save, X, Plus, Trash2, Clock, Link as LinkIcon, CheckCircle, GripVertical, History } from 'lucide-react';
...
export default function TemplatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'templates', id), (docSnap) => {
      if (docSnap.exists()) {
  ...
  const acquireLock = async () => {
    ...
    if (lock && lock.userId !== user.uid) {
      toast.error(`Este modelo está sendo editado por ${lock.userName}.`);
      return false;
    }

    try {
      await setDoc(doc(db, 'locks', id), {
        scheduleId: id, // Using scheduleId for consistency in locks collection
        userId: user.uid,
      ...
      <header className={`sticky top-0 z-50 px-4 py-3 flex items-center justify-between shadow-sm ${isEditing ? 'bg-[#E2A065] text-white' : 'bg-white'}`}>
        ...
        <div className="flex flex-col items-center">
          <span className="font-bold text-sm">
            {isEditing ? 'Editando Modelo' : 'Visualizando Modelo'}
          </span>
          {lock && !isEditing && (
        ...
        {isEditing ? (
          <div className="space-y-6">
            <div className="space-y-4">
              <div className="relative bg-white rounded-2xl p-4 shadow-sm">
                <label className="text-xs font-bold text-[#e6614c] uppercase">Título do Modelo</label>
                <input 
                  value={title}
          ...
          <div className="space-y-8">
            <div className="text-left">
              <h1 className="text-4xl font-bold text-[#4A3F35] leading-tight mb-2">{template?.title}</h1>
              <p className="text-[#e6614c] font-bold text-lg">Modelo de Cronograma</p>
            </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-gray-100 p-4 flex gap-4 max-w-lg mx-auto w-full z-40">
        {isEditing ? (
          ...
          <button 
            ...
            className="flex-1 bg-[#e6614c] text-white rounded-full font-bold shadow-lg flex items-center justify-center gap-2 py-4 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Edit3 size={20} />
            Editar Modelo
          </button>
    ...
    <div className="space-y-5">
      <div className="flex gap-4">
        <div className="flex-1 bg-[#FAF6F0] rounded-2xl p-4">
          <label className="text-xs font-bold text-[#9CA3AF] uppercase">Horário Sugerido (Opcional)</label>
          <input 
            type="time"
            value={time}
